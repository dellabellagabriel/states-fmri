...
for iSub, sub in enumerate(subs[0:4]):
    ...
    for iCond in range(1,5):
        ...
        #img = nib.Nifti1Image(he_voxel, np.eye(4))
        #nib.save(img, f"/home/usuario/disco1/proyectos/2023-resting-state-estados-fMRI_conn/results/permutation_entropy/he/cond{iCond}/he-{os.path.basename(sub)}-cond{iCond}.nii")
        #img = nib.Nifti1Image(sc_voxel, np.eye(4))
        #nib.save(img, f"/home/usuario/disco1/proyectos/2023-resting-state-estados-fMRI_conn/results/permutation_entropy/sc/cond{iCond}/sc-{os.path.basename(sub)}-cond{iCond}.nii")