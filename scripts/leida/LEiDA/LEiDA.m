...
if isempty(varargin) % If no input is given, compute LEiDA
    ...
    for k=1:length(rangeK)      
        ...
        [IDX, C, SUMD, D]=kmeans(Leading_Eig,rangeK(k),'Replicates',100,'MaxIter',1000,'Display','off','Options',statset('UseParallel',1));
...
for c=1:K
    ...
    ProbC(c)=histogram_entropy(Best_Clusters.C(c,:)' * Best_Clusters.C(c,:));
end
...
for c=1:K
    ...
    subplot(4,K,2*K+c)  
            ...
            Transicion=squeeze(P(2,:,k,ind_sort(c)));
            Alteracion=squeeze(P(3,:,k,ind_sort(c)));
            Recuperacion=squeeze(P(4,:,k,ind_sort(c)));
            bar([mean(Rest) mean(Transicion) mean(Alteracion) mean(Recuperacion)],'EdgeColor','w','FaceColor',[.5 .5 .5])
            hold on
            % Error bar containing the standard error of the mean
            errorbar([mean(Rest) mean(Transicion) mean(Alteracion) mean(Recuperacion)],[std(Rest)/sqrt(numel(Rest)) std(Transicion)/sqrt(numel(Transicion)) std(Alteracion)/sqrt(numel(Alteracion)) std(Recuperacion)/sqrt(numel(Recuperacion))],'LineStyle','none','Color','k')
            set(gca,'XTickLabel',{'Rest', 'Trans', 'Alter', 'Recu'})
            if P_pval(k,ind_sort(c))<0.05
                plot(1.5,max([mean(Rest) mean(Transicion) mean(Alteracion) mean(Recuperacion)])+.01,'*k')
            end             
            if c==1
                ylabel('Probability')
            end
     ...
     subplot(4,K,3*K+c)  
            ...
            Transicion=squeeze(LT(2,:,k,ind_sort(c)));
            Alteracion=squeeze(LT(3,:,k,ind_sort(c)));
            Recuperacion=squeeze(LT(4,:,k,ind_sort(c)));
            bar([mean(Rest) mean(Transicion) mean(Alteracion) mean(Recuperacion)],'EdgeColor','w','FaceColor',[.5 .5 .5])
            hold on
            errorbar([mean(Rest) mean(Transicion) mean(Alteracion) mean(Recuperacion)],[std(Rest)/sqrt(numel(Rest)) std(Transicion)/sqrt(numel(Transicion)) std(Alteracion)/sqrt(numel(Alteracion)) std(Recuperacion)/sqrt(numel(Recuperacion))],'LineStyle','none','Color','k')
            set(gca,'XTickLabel',{'Rest', 'Trans', 'Alter', 'Recu'})
